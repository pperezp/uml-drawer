...
public class UMLDrawer extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        codigo_diagrama.setFont(new java.awt.Font("DejaVu Sans", 0, 18)); // NOI18N
...
//        panelDiagrama.addActor(new Actor(1,"Actor 1"));
        ...
        for (String lin : lineas) {
            if (lin.contains("-")) {
                item1 = lin.split("-")[0].trim();
                item2 = lin.split("-")[1].trim();

                if ((item1.startsWith("[") && item1.endsWith("]"))
                        && (item2.startsWith("(") && item2.endsWith(")"))) {
                    // actor y caso de uso
                    nomActor = item1.replace("[", "").replace("]", "").trim();
                    nomCaso = item2.replace("(", "").replace(")", "").trim();
